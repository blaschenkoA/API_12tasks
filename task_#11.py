...
pygame.display.set_caption('Maps API task #11')
...
class Maps_api:
    ...
    def mouse_btn(self, event):
        ...
        if event.pos[0] in range(20, 620) and event.pos[1] in range(40, 490) and \
                any(map(float, self.text_coord.split(','))):
            self.locate_map(event.pos)
        if self.sign_in_button.collidepoint(event.pos):
            ...
            if '[' in self.text_adres:
                ...
                toponym = requests.get(map_request).json()["response"]["GeoObjectCollection"]["featureMember"][0]["GeoObject"]
    ...
    def locate_map(self, cords):
        cords = [cords[0] - 320, cords[1] - 265]

        map_request = "https://search-maps.yandex.ru/v1/"

        # Вот это не трогать, прям вообще, иначе укушу ((
        map_params = {'apikey': "dda3ddba-c9ea-4ead-9010-f43fbc15c6e3",
                      'text': ','.join(
                          map(str, [float(self.text_coord.split(',')[1]) + int(self.text_map) / 600000 * cords[1],
                                    float(self.text_coord.split(',')[0]) + int(self.text_map) / 600000 * cords[0]])),
                      'rspn': 1,
                      'll': ','.join(
                          map(str, [float(self.text_coord.split(',')[0]) + int(self.text_map) / 600000 * cords[0],
                                    float(self.text_coord.split(',')[1]) + int(self.text_map) / 600000 * cords[1]])),
                      'lang': 'ru_RU'}

        self.organization_response = requests.get(map_request, map_params).json()
